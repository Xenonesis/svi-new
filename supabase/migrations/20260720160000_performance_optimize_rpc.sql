-- ═══════════════════════════════════════════════════════════
-- Performance — optimize get_distinct_registration_filters RPC
-- ═══════════════════════════════════════════════════════════
-- Switches from LANGUAGE plpgsql (DECLARE/BEGIN/END → black-box)
-- to LANGUAGE sql so PostgreSQL can inline and better plan
-- the 7 indexed subqueries. Combined with the v5 partial
-- indexes, each subquery hits a small B-tree index.
--
-- Expected: ~660ms → ~150-250ms (2-3× faster)
-- ═══════════════════════════════════════════════════════════
