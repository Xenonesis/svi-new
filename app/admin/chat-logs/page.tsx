...
function getDateRangeLabel(dateFrom: string, dateTo: string): string {
  ...
  const from = dateFrom
    ...
    : 'Start';
  const to = dateTo
    ? new Date(dateTo).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })
    : 'Now';
...
function downloadCSV(logs: ChatLog[]) {
  const header = 'Session ID,Created,Updated,Messages,User Messages,Preview';
...
export default function ChatLogsPage() {
  ...
  const fetchLogs = useCallback(
    async (pageNum: number, appendStats = false) => {
      if (!token) return;
      setLoading(true);
      setError('');
      ...
      try {
        ...
        // Compute stats (only on first page)
        if (appendStats || pageNum === 1) {
          ...
          // Fetch stats from separate endpoints or compute from available data
          const todayRes = await fetch(
            `/api/chat/log?${new URLSearchParams({
              page: '1',
              limit: '1',
              sort_by: 'updated_at',
              sort_order: 'desc',
          ...
          setTodayCount(todayData.pagination?.total || 0);
          ...
          setWeekCount(weekData.pagination?.total || 0);

          if (total > 0) {
            // Get avg messages from one page sample
            const avgMsgs =
              data.logs?.reduce(
                (sum: number, l: ChatLog) =>
                  sum + (l.message_count || parseMessages(l.messages).length),
                0
              ) / Math.max(data.logs.length, 1);
            setAvgMessages(Math.round(avgMsgs));
          }
        }
      } catch {
  ...
  // Initial load
  useEffect(() => {
    if (token && mounted) fetchLogs(1, true);
  ...
  const handleDeleteSelected = useCallback(async () => {
    ...
    try {
      for (const id of selectedIds) {
        await fetch(`/api/chat/log?id=${id}`, {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}` },
        });
      }
    ...
    <div className="mx-auto max-w-7xl">
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Track and review AI chatbot conversations
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={loading}
            className="flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-600 transition-colors hover:bg-gray-50 disabled:opacity-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700"
          >
        ...
        {[
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-xl font-bold text-gray-900 dark:text-white">{stat.value}</p>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="flex items-center gap-2">
          {selectedIds.size > 0 && (
            ...
            >
              ...
              <button
                ...
                className="flex items-center gap-1 rounded-lg bg-red-50 px-3 py-1.5 text-sm font-medium text-red-600 transition-colors hover:bg-red-100 dark:bg-red-950/30 dark:text-red-400 dark:hover:bg-red-950/50"
              >
          ...
          {pagination.total > 0 && (
            <button
              ...
              className="flex items-center gap-1 rounded-lg px-3 py-1.5 text-xs text-gray-400 transition-colors hover:text-red-500"
            >
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1 sm:flex-initial">
            ...
            <input
              ...
              className="focus:border-brand-gold focus:ring-brand-gold/20 dark:focus:border-brand-gold w-full rounded-lg border border-gray-200 bg-white py-2 pr-4 pl-10 text-sm transition-colors outline-none focus:ring-2 sm:w-64 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200"
            />
            {search && (
              <button
                onClick={() => setSearch('')}
          ...
          >
            ...
            Filters
            {(dateFrom || minMsgs) && (
              <span className="bg-brand-gold ml-1 flex h-4 w-4 items-center justify-center rounded-full text-[10px] font-bold text-white">
                {(dateFrom ? 1 : 0) + (minMsgs ? 1 : 0)}
              </span>
            )}
          </button>
        </div>
            ...
            <div className="mb-4 rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="focus:border-brand-gold w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200"
                    />
                    <span className="flex items-center text-xs text-gray-400">to</span>
                    <input
                      type="date"
                      value={dateTo}
                    ...
                    {DATE_PRESETS.map((p) => (
                      <button
                        ...
                        className={`rounded-full px-2.5 py-1 text-[11px] font-medium transition-colors ${
                          ...
                          (p.days > 0 &&
                            dateFrom ===
                              new Date(Date.now() - p.days * 86400000).toISOString().split('T')[0])
                            ? 'bg-brand-gold/15 text-brand-navy dark:text-brand-gold'
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-xs text-gray-600 transition-colors hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400"
                    >
                ...
                <div className="flex items-end gap-2">
                  <button
                    ...
                    className="bg-brand-navy hover:bg-brand-navy-light dark:bg-brand-gold dark:text-brand-navy flex-1 rounded-lg py-2 text-sm font-medium text-white transition-colors"
                  >
                    Apply Filters
                  </button>
                  <button
                    ...
                    className="rounded-lg border border-gray-200 px-4 py-2 text-sm text-gray-500 transition-colors hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-700"
                  >
                    Reset
                  </button>
                </div>
              ...
              {(dateFrom || minMsgs) && (
                <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-gray-100 pt-3 dark:border-gray-700">
                  ...
                  {dateFrom && (
                    <span className="bg-brand-gold/10 text-brand-navy dark:text-brand-gold inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs">
                  ...
                  {minMsgs && (
                    <span className="inline-flex items-center gap-1 rounded-full bg-purple-50 px-2.5 py-1 text-xs text-purple-700 dark:bg-purple-950/30 dark:text-purple-400">
                      Messages: {minMsgs}
                      {maxMsgs ? ` — ${maxMsgs}` : '+'}
                      <button
                        onClick={() => {
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-24">
          <div className="border-brand-gold h-10 w-10 animate-spin rounded-full border-4 border-t-transparent" />
      ...
      {/* ── Error ────────────────────────────────────────────────────────── */}
      {error && !loading && (
        <div className="flex flex-col items-center justify-center rounded-xl border border-red-200 bg-red-50 py-16 dark:border-red-800 dark:bg-red-950/30">
      ...
      {!loading && !error && filteredLogs.length === 0 && (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-200 py-24 dark:border-gray-700">
          ...
          <p className="text-sm font-medium text-gray-900 dark:text-white">
            {search || dateFrom ? 'No conversations match your filters.' : 'No chat logs yet.'}
          </p>
          <p className="mt-1 text-xs text-gray-500">
            {search || dateFrom
              ? 'Try adjusting your search or filters.'
              : 'Chat logs will appear here once users interact with the AI chatbot.'}
          </p>
          {(search || dateFrom) && (
            <button
              onClick={() => {
            ...
            {filteredLogs.map((log) => {
              ...
              const preview =
                lastAiMsg?.parts
                  ?.map((p: any) => p.text || '')
                  .join(' ')
                  ?.slice(0, 150) || '(empty response)';
              const userCount =
                log.user_message_count ?? msgs.filter((m: any) => m.role === 'user').length;
              ...
              return (
                <div key={log.id} className="group">
                  <div
                    className={`flex items-start gap-3 rounded-xl border bg-white p-4 transition-all dark:bg-gray-800 ${
                      isSelected
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex min-w-0 items-center gap-3">
                          <div className="bg-brand-gold/10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full">
                            <MessageCircle className="text-brand-gold h-4 w-4" />
                          </div>
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              ...
                              <span
                                className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ${
                                  formatDate(log.updated_at) === 'Today'
                                    ? 'bg-green-50 text-green-700 dark:bg-green-950/30 dark:text-green-400'
                                    : formatDate(log.updated_at) === 'Yesterday'
                            ...
                            <div className="mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-500">
                              ...
                              <span className="flex items-center gap-1">
                                <Bot className="text-brand-gold h-3 w-3" />
                  ...
                  <AnimatePresence>
                    {isSelected && (
                      <motion.div
                        ...
                        className="overflow-hidden"
                      >
                        <div className="mx-2 mb-2 max-h-[500px] overflow-y-auto rounded-xl border border-gray-100 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-900">
                                ...
                                >
                                  <div
                                    className={`max-w-[85%] rounded-2xl px-4 py-2.5 ${
                                      isUser
                                        ? 'bg-brand-navy rounded-tr-md text-white'
                                  ...
                                  >
                                    <div className="mb-1 flex items-center gap-2">
                                      ...
                                      <span className="text-[10px] opacity-60">
                                        {isUser ? 'User' : 'AI'} ·{' '}
                                        {formatTime(m.createdAt || log.created_at)}
                                      </span>
                                    </div>
        ...
        {showConfirmDelete && (
          ...
          >
            <motion.div
              ...
              className="mx-4 w-full max-w-sm rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-gray-700 dark:bg-gray-800"
            >
