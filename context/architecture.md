...
- **Durable Email Archiving:** The Admin Email Center (`/admin/email`) uses Supabase `email_messages` as the permanent, immutable store for all outbound sent emails (decoupling email history from Resend's 30-day log purge policy). All sent emails via Compose, scheduled jobs, and campaigns persist full HTML/text content, recipient lists, and metadata directly in PostgreSQL, while inbound messages are mirrored into `email_inbox`.
