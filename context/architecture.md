...
- **Durable Email Archiving & Realtime Inbox:** The Admin Email Center (`/admin/email`) uses Supabase `email_messages` as the permanent, immutable store for all outbound sent emails (decoupling email history from Resend's 30-day log purge policy). Inbound customer replies are captured via Resend webhook (`/api/webhooks/resend/incoming`), mirrored into `email_inbox`, and synced instantly to the Admin Inbox via a Supabase Realtime subscription on `email_inbox`. Outbound email Reply-To routing points to verified domain `info@sviiinfrasolutions.com` ensuring MX records route client replies back to Resend.
...
- **Employee Directory & Performance Tracking:** Admin manages the complete employee lifecycle and views individual performance dashboards via `/admin/workforce?tab=directory` (legacy `/admin/employees` redirects here), tracking KPIs for attendance, punctuality, conversion rate, site visits, assigned leads with chronological interaction timelines, dynamic lead reassignment transfers (`/api/admin/employees/[id]/leads`), admin task & target assignment (`AdminEmployeeTasksView` & `/api/admin/employees/[id]/tasks`), and daily shift work logs auditing (`AdminEmployeeLogsView` & `/api/admin/employees/[id]/logs`). On the employee side, `/employee/work` provides dynamic project selection (`AddLeadModal`), stage-by-stage pipeline tracking with interaction logs (`LeadTrackerDrawer`), site visits scheduling & GPS check-in, daily shift log submissions, native Web Audio melodic chime reminders (`followUpAudio`), and automated Admin notification dispatching when follow-ups are scheduled.
