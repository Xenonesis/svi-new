...
| `properties`      | Main property listings and details, including dynamic legal land locations (`legal_location_hi`, `legal_location_en`, `location`) for BBA and Allotment documents |
...
| `idx_chat_leads_phone_source_unique`            | `chat_leads(phone, source) UNIQUE`                         | Guarantees idempotent upserts for multi-channel leads    |
| `idx_chat_leads_status_created`                 | `chat_leads(lifecycle_status, created_at DESC)`            | Fast pipeline stage filtering & sorting                  |
| `idx_ivr_call_records_agent_dial_time`          | `ivr_call_records(assigned_agent_id, dial_time DESC)`      | Fast advisor filtering & sorting in IVR leads table      |
| `idx_ivr_call_records_status_dial_time`         | `ivr_call_records(dial_status, dial_time DESC)`            | Fast dial status filtering & sorting in IVR leads table  |
| `idx_ivr_call_records_time_status`              | `ivr_call_records(dial_time DESC, dial_status)`            | Fast telemetry and time-cutoff performance queries       |
| `idx_ivr_call_records_phone_trgm`               | `ivr_call_records USING gin (customer_phone gin_trgm_ops)` | Instant customer phone search in IVR table               |
| `idx_ivr_call_records_agent_name_trgm`          | `ivr_call_records USING gin (agent_name gin_trgm_ops)`     | Instant agent name search in IVR table                   |
| `idx_lead_interactions_phone_created`           | `lead_interactions(lead_phone, created_at DESC)`           | Sub-1ms timeline activity lookups for lead drawer        |
