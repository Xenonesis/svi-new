...
// Assuming the types are inferred or imported. For simplicity, we define the props here.
export function AllotmentLetterForm({
  formData,
  setFormData,
  advisors,
  projects,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {isCustomAdvisor ? (
          ...
          <FormSelect
            ...
            options={[
              ...
              ...advisors.map((adv: any) => ({ value: adv.full_name, label: adv.full_name })),
              { value: 'custom', label: 'Other / Custom...' },
            ]}
          />
        )}
      ...
      <button
        type="submit"
        disabled={isSaving}
        className={`bg-brand-gold hover:bg-brand-gold-light text-brand-navy glow-gold mt-4 flex w-full items-center justify-center gap-2 rounded-lg py-3.5 text-xs font-bold tracking-widest uppercase shadow-lg transition-all ${
