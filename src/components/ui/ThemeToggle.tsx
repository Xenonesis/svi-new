...
export function ThemeToggle({
  ...
  return (
    <button
      onClick={onToggle}
      className={`touch-lux group relative flex min-h-[44px] min-w-[44px] items-center justify-center rounded-full border p-2 transition-all duration-300 hover:shadow-sm xl:p-2.5 ${variantClass}`}
      title={getThemeLabel()}
      aria-label={
        mounted
          ? theme === 'dark'
