...
export default function EMIResults({
  ...
  const headlineLabel = isOneTime
    ? 'TOTAL OUTFLOW · FULL PAYMENT'
    : isNoCostEmi
  ...
  return (
    <div className="dark:border-brand-gold/30 relative overflow-hidden rounded-2xl border border-amber-500/30 bg-slate-900 p-6 text-white shadow-2xl md:p-8">
      ...
      {isOneTime && (
        <div className="mb-4 inline-flex items-center gap-1.5 rounded-full bg-amber-500/15 px-3 py-1 text-[10px] font-bold tracking-widest text-amber-400 uppercase">
          <Tag size={12} />
          One-Time Payment Plan (Best Rate)
        </div>
      )}

      {isNoCostEmi && !isOneTime && (
      ...
      <div className="mb-6">
        ...
        <div className="dark:text-brand-gold mt-2 font-serif text-4xl font-bold text-amber-400 md:text-5xl">
          ...
          <span className="ml-2 font-sans text-xs font-normal text-gray-300">
            {isOneTime ? '(One-Time)' : paymentMonths ? `/ month × ${paymentMonths} mo` : '/ month'}
          </span>
        </div>
        {isOneTime ? (
          <p className="mt-2 text-[10px] text-amber-300/80">
            Best upfront price · Zero interest · Direct registry ownership
          </p>
        ) : isNoCostEmi ? (
      ...
      <div className="space-y-4 border-t border-white/10 pt-4 text-xs">
        <div className="flex items-center justify-between text-gray-300">
          <span>{isOneTime ? 'Total Plot Cost:' : 'Principal Amount:'}</span>
        ...
        <div className="flex items-center justify-between text-gray-300">
          ...
          <span className="font-semibold text-white">
            {isOneTime || isNoCostEmi ? '₹ 0' : `₹ ${totalInterest.toLocaleString('en-IN')}`}
          </span>
        </div>
      ...
      <div className="mt-6 flex items-center justify-between border-t border-white/10 pt-4">
        ...
        >
          {isOneTime ? 'Book Plot Directly' : isNoCostEmi ? 'Book with No-Cost EMI' : 'Apply Loan'}
        </Link>
      </div>
    </div>
  );
